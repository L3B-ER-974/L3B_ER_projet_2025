...
unsigned int Vitesse_m_s = 1000; // 0,00ms = 25,00m/s
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(5), pulseCounter, FALLING); // Interruption sur front descendant
}
...
void loop() {
  if (millis() - lastTime >= 1000) { // Toutes les secondes
    ...
    Vitesse_m_s = int(flowRate) ; // Vitesse en m/s en entier
  ...
  CAN.write((Vitesse_m_s >> 8) & 0x00FF);         // MSB Vitesse relative sur l'eau x0.01m/s
  CAN.write(0xFF);
  CAN.write(0xFF);
  CAN.write(0xFF);
  CAN.write(0xFF);
